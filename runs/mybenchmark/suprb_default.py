...
Aufruf (direkt):
    python suprb_default.py -p airfoil_self_noise -n 32 -r 4 -i 4
...
#TODO Journal Storage
#JOURNAL_PATH = os.path.join(_HERE, "optuna_journals") 
...
def run(
    ...
    print(f"[run] Problem={problem}  job_id={job_id}  n_iter={n_iter}  "
        f"n_rules={n_rules}  n_initial_rules={n_initial_rules}  tune={tune}")
    ...
    study_name = f"{problem}__ni{n_iter}__nr{n_rules}__nir{n_initial_rules}__{tune_label}"
    ...
    if tune: 
        ...
        #TODO: maybe change to journalStorage 
        #journal_path = os.path.join(JOURNAL_DIR, f"{problem}.log")
        ...
        tuned_params = run_tuning(
            ...
            storage_url=db_url,
            #storage=storage,   #TODO journal storage
            n_trials=1000,
    ...
    experiment_name = f"SupRB | problem={problem} | {tune_label}" #for airfoil ds, label entfernen-> renamed airfoil mlflow experiment
